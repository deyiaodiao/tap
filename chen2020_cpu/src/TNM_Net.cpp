...
std::atomic<int> TNM_SPATH::nextPathID{1};
...
TNM_SPATH::TNM_SPATH()
{
	id = nextPathID.fetch_add(1, std::memory_order_relaxed);
	flow       = 0.0; 
	cost       = 0.0; 
	//active     = true;
	//reIte      =0;
...
int TNM_SNET::BuildTAPAS(bool loadod, const TNM_LINKTYPE ltype)
{
	...
	if(loadod && nz >0)
	{
		...
		for(int i = 0;i<nz;i++)
		{
			// Some TNTP data sets declare the number of zones rather than the
			// number of Origin blocks.  Stop at the real end of the trips file;
			// otherwise the failed extraction leaves orgID unchanged and creates
			// duplicate, empty copies of the final origin.
			if(!(odFile>>orgID)) break;
			if((i+1)%100 ==0)
			{
